...
class ReadingListView extends StatefulWidget {
  ...
  final Map<String, bool> selectedReadings;
  final Function(String) onToggleSelection;
  final bool isMultipleMode;
  final String currentlyPlayingMultipleTitle;
  ...
  const ReadingListView({
    ...
    required this.isMultipleMode,
    required this.currentlyPlayingMultipleTitle,
  });
    ...
    if (widget.isEditMode && widget.onReorder != null) {
      ...
      return ReorderableListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final isPlayingMultiple =
              (reading['title'] == widget.currentlyPlayingMultipleTitle);
          final isSelected = widget.selectedReadings[reading['title']] ?? false;
          ...
          return ListTile(
            ...
            leading: widget.isEditMode
                ...
                : Checkbox(
                    ...
                    onChanged: (bool? value) {
                      widget.onToggleSelection(reading['title']);
                    },
                  ),
            title: Text(
              reading['title'],
              style: TextStyle(
                color: (isPlaying || isPlayingMultiple)
                    ? Colors.blue
                    : Colors.black,
                fontWeight: (isPlaying || isPlayingMultiple)
                    ? FontWeight.bold
                    : FontWeight.normal,
              ),
            ),
            trailing: Row(
              ...
              children: [
                ...
                if (isPlaying || isPlayingMultiple)
                  _buildAnimatedPlayingIndicator(),
          ...
          return Column(
            children: [
              ListTile(
                ...
                onTap: () {
                  // Click to play/stop reading directly (only if not in multiple mode)
                  if (!widget.isMultipleMode) {
                    if (!isPlaying) {
                      widget.onPlayReading(
                          reading['title'], reading['content']);
                    } else {
                ...
                trailing: _buildTrailingWidget(
                    reading, isPlaying || isPlayingMultiple, widget.isEditMode),
              ),
